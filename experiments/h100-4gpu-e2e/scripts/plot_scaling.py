#!/usr/bin/env python3
"""Plot matched TeleFuser MiniMax-H3 Base one- and four-GPU results."""
...
def main() -> None:
    ...
    panels = (
        ("Denoise time", "seconds / video", [one["denoise_seconds"], four["denoise_seconds"]]),
        (
            "Denoise throughput",
            "steps / second",
            [one["config_points_per_second"], four["config_points_per_second"]],
        ),
        (
            "Peak memory",
            "GiB / GPU",
            [
                one["representative_peak_memory_mib"] / 1024.0,
                four["representative_peak_memory_mib"] / 1024.0,
            ],
        ),
    )
    ...
    text(
        root,
        "4 GPUs: TP2 × Ulysses SP2 · feature cache disabled · warm request",
        width / 2,
        585,
